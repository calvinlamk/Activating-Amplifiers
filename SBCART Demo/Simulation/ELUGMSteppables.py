...
class ELUGMSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.pW2 = self.addNewPlotWindow(
            ...
            _yScaleType='linear',
            _grid=True)                
        self.pW2.addPlot('Kills', _style='Dots', _color='white', _size=3)
        ...
        global YtoY,YtoG,GtoY,YtoB,BtoY,YtoR,RtoY,GtoG,GtoB,BtoG,GtoR,RtoG,BtoB,BtoR,RtoB,RtoR,OtoY,YtoO,OtoG,GtoO,OtoB,BtoO,OtoR,RtoO,OtoO,WtoY,YtoW,WtoG,GtoW,WtoB,BtoW,WtoR,RtoW,WtoO,OtoW,WtoW 
...
#                 print CSAY, CSAG, PTSG, CSAB, PTSB, CSAR, PTSR, CSAM
            ...
            if (cell.type==1 or cell.type==2 or cell.type==3 or cell.type==6):
                DTREPO=(1/(1+np.exp(-((PTSL+PTSK)-BETA))))-(1/KAPPA)*cell.dict["PTS"][0]         #GFP       GRBY 0123
                cell.dict["PTS"][0]+=DTREPO                                                          
                DTREPT=(1/(1+np.exp(-((cell.dict["PTS"][0])-BETA))))-(1/KAPPA)*cell.dict["PTS"][2]    #BFP 
                cell.dict["PTS"][2]+=DTREPT                                                                                                      
            if (cell.type==4 or cell.type==8):
                DTREPH=(1/(1+np.exp(-((PTSB+PTSW)-BETA))))-(1/KAPPA)*cell.dict["PTS"][3]           #YFP  
                cell.dict["PTS"][3]+=DTREPH              
                                                             
            if (cell.type==1 or cell.type==2 or cell.type==3 or cell.type==6): #FACS PLOTS AND CADHERIN LINKAGE TO STATE
                ...
                if cell.dict["PTS"][0]>=THRESHOLD and cell.dict["PTS"][2]>=THRESHOLD:   #GFP+ BFP+ state
                    cell.type=6
                if cell.dict["PTS"][0]<THRESHOLD and cell.dict["PTS"][2]<THRESHOLD:   #GFP- BFP- state gray cells
                    cell.type=1
            if (cell.type==4 or cell.type==8):
                if cell.dict["PTS"][3]>=THRESHOLD:
        ...
        if mcs%RESOL==0:
            global KILLS
            self.pW2.addDataPoint("Kills", mcs, KILLS)
            ...
            if (NUMTO+NUMTL+NUMTR+NUMTK)==0:
                self.pW4.addDataPoint("YFP", mcs, 0)
            if (NUMTO+NUMTL+NUMTR+NUMTK)>0:
